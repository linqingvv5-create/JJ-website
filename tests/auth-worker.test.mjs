...
class FakeDb {
  ...
  prepare(sql) {
    ...
    return {
      ...
      async all() {
        if (sql.includes("SELECT member_id FROM finance_member_locks")) return { results: [...db.locks.keys()].map((member_id) => ({ member_id })) };
        return { results: [] };
      },
      async first() {
...
test("password session protects API routes", async () => {
  ...
  const setup = await worker.fetch(new Request(`${base}/api/member-locks/member-me/password`, {
    method: "PUT",
  ...
  const wrongMemberPassword = await worker.fetch(new Request(`${base}/api/member-locks/member-me/unlock`, {
    method: "POST",
    headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
  ...
  const memberLogin = await worker.fetch(new Request(`${base}/api/member-locks/member-me/unlock`, {
    method: "POST",
    headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
