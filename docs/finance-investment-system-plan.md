...
## 12. 财务计算口径与数据一致性（2026-07-21 验收）

### 家庭账本交易规则

- `INCOME`（收入）：增加转入账户余额；计入月度收入，不计入支出。
- `EXPENSE`（支出）：减少转出账户余额；计入月度支出，不计入收入。信用卡消费也使用该类型。
- `TRANSFER`（转账）：必须同时填写不同的转出、转入账户；一减一增，收入和支出均不变化。银行卡与证券账户互转、信用卡还款、银行卡转入鹅鸭鸡关联账户都属于转账。
- `REFUND`（退款）：增加转入账户余额并冲回家庭资金，当前月报口径计入收入侧的退款金额；它仍保持独立类型，不伪装成普通收入。
- `REIMBURSEMENT`（报销）：增加转入账户余额，当前月报口径计入收入侧的报销金额；它仍保持独立类型。
- `BALANCE_ADJUSTMENT`（余额调整）：仅按带正负号的调整差额修正账户余额，不计收入或支出。
- 股票买入、卖出、持仓和投资盈亏只由投资模块管理，不生成 `FinanceTransaction`，不进入普通家庭收入或支出。

### 家庭净资产公式

`家庭净资产 = 日常计入资产账户余额 + Σ投资摘要总资产 - 信用卡负债`

其中：

- 日常账户只汇总 `includeInFamilyAssets=true`、未归档且不是信用卡、证券账户镜像的账户。
- 证券账户及通过 `externalInvestmentAccountId` 映射到投资摘要的账户不在日常账户侧重复相加。
- 投资摘要总资产已经包含证券账户内部可用资金，不再额外加一次“投资可用资金”。
- 银行卡资金只在日常账户侧出现一次；由银行卡转入证券账户时应同时更新真实资金来源，且转账本身不改变家庭净资产。
- 信用卡负余额按负债扣除，信用卡还款只是账户转账。

### `InvestmentAccountSummary` 字段定义

- `investmentAccountId`：原投资模块的账户标识。
- `name`：投资账户名称。
- `marketValueCents`：该账户持仓的当前市值，不包含现金。
- `availableCashCents`：仍留在该投资账户内部、可用于交易的现金。
- `totalAssetCents`：投资账户总资产，严格等于 `marketValueCents + availableCashCents`。
- `profitLossCents`：投资持仓盈亏；遵循 A 股红涨绿跌，不计普通家庭收入。
- `principalCents`：当前阶段由 `totalAssetCents - profitLossCents` 推算的投入本金；原投资数据增加完整出入金流水后可改为实算累计本金。
- `updatedAt`：摘要本次从投资模块读取并计算的时间。
...
- 鸭（🦆）：中长期目标。
- 鸡（🐔）：短期愿望和计划。
- 当前金额低于目标金额时，完成率最高显示 `99.99%`，不会因四舍五入提前显示 `100.00%`；完成状态也保持进行中或存钱中。

### 场景验收结果

- 场景 A：微信支付 35 元，分类“生活必须支出 / 餐饮”，归属家庭。测试通过：微信余额减少 35 元；月度支出增加 35 元；结余和家庭净资产各减少 35 元。
- 场景 B：银行卡转入买房鸭 10,000 元。测试通过：银行卡减少 10,000 元；目标关联账户和买房鸭进度增加 10,000 元；家庭净资产、收入、支出均不变化。
- 场景 C：投资总资产由 999,789.95 元调整为 980,000 元。测试通过：小鹅进度由 99.98% 变为 98.00%；家庭净资产减少 19,789.95 元；没有新增家庭账目，月度收入和支出均不变化。

### 保存与跨设备能力

线上版本使用 Sites D1 保存一份家庭共享状态，能够让访问同一站点的设备读取同一份云端数据，因此具备基础的跨设备共享保存。但当前没有家庭账号登录、成员身份鉴权、逐用户数据隔离和并发冲突合并，不能称为完整的账号级同步。界面统一显示“已保存到云端共享账本”或“已保存在此设备”，不再使用含义过度的“已同步”。
